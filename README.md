...
`sitemap.xml` automatically includes pages using the `homepage` or `default` layout, except pages with `sitemap: false`. It does not invent modification dates. `robots.txt` allows crawlers to load the pages and their rendering resources.
