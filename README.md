...
Page titles, descriptions, canonical URLs, and sharing metadata are generated by `_includes/seo.html`. The About page also describes the site owner with `ProfilePage` / `Person` structured data and links to the existing academic profiles. Keep these details consistent with the visible biography. Each main page has its own description in its front matter.

`sitemap.xml` automatically includes pages using the `homepage` or `default` layout, except pages with `sitemap: false`. It does not invent modification dates. `robots.txt` allows crawlers to load the pages and their rendering resources. Teaching and service now share `/activities/`; the old `/teaching/` and `/services/` addresses immediately redirect to their corresponding sections and are excluded from the sitemap.

After publishing, use [Google Search Console](https://search.google.com/search-console) to verify the URL-prefix property `https://shaoyanliu.github.io/`. If choosing HTML-tag verification, copy only the provided tag's `content` value into `google_site_verification` in `_config.yml`, publish it, and complete verification in Search Console. The verification value is a public website tag, not an API key. An empty value emits no verification tag.

Submit `https://shaoyanliu.github.io/sitemap.xml`, inspect the homepage URL, and request indexing after substantial changes. Use the Performance report to monitor name searches such as `Shaoyan Liu` and `Shaoyan Liu Penn State`. Verification, sitemap submission, and structured data do not guarantee indexing or a particular ranking. Keep links to this homepage on relevant institutional and academic profiles current.
